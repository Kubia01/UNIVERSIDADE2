...
const Sidebar: React.FC<SidebarProps> = ({ activeView, onViewChange, isOpen, onClose, user }) => {
  const getMenuItems = () => {
    if (user?.role === 'admin') {
      return [
        ...
        { id: 'content', icon: FileText, label: 'Atribuição de Cursos' },
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 truncate">
                  {user?.department || 'Departamento'} • {user?.role === 'admin' ? 'Administrador' : 'Colaborador'}
                </p>
              </div>
            </div>
          </div>
