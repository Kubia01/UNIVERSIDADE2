...
import { ArrowLeft, Upload, Plus, Trash2, Save, Video, FileText, HelpCircle, Link, Edit } from 'lucide-react';
...
const CourseCreation: React.FC<CourseCreationProps> = ({ course, onBack, onSave }) => {
  ...
  const handleAddLesson = () => {
    ...
    const newLesson = {
      ...currentLesson,
      id: editingLessonIndex !== null ? courseData.lessons[editingLessonIndex].id : Date.now().toString()
    };
          ...
          {showLessonForm && (
            <div className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    {currentLesson.type === 'video' ? 'URL do Vídeo ou Upload' : 
                     currentLesson.type === 'document' ? 'URL do Documento/PDF ou Upload' :
                     currentLesson.type === 'link' ? 'URL do Link' : 'Conteúdo ou Upload'}
                  </label>
                    ...
                    <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-4">
                      <div className="text-center">
                        ...
                        <label className="cursor-pointer">
                          <span className="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-500">
                            Clique para fazer upload do arquivo
                          </span>
                          <input
                            type="file"
                            accept={currentLesson.type === 'video' ? 'video/*' : 
                                   currentLesson.type === 'document' ? '.pdf,.doc,.docx,.ppt,.pptx' : 
                                   '*/*'}
                            onChange={(e) => {
                              const file = e.target.files?.[0];
                              if (file) {
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                          {currentLesson.type === 'video' ? 'Formatos: MP4, AVI, MOV' :
                           currentLesson.type === 'document' ? 'Formatos: PDF, DOC, DOCX, PPT, PPTX' :
